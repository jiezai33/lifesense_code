    ...
    switch (p_ble_evt->header.evt_id)
    {
        ...
        case BLE_GAP_EVT_TIMEOUT:
            if (p_ble_evt->evt.gap_evt.params.timeout.src == BLE_GAP_TIMEOUT_SRC_ADVERTISING)
            {
                ...
                {
                    case BLE_ADV_MODE_DIRECTED:
                        QPRINTF("[ADV]: Timed out from directed advertising.\r\n");
                        {
                            uint32_t err_code;
                        ...
                        break;
                    case BLE_ADV_MODE_DIRECTED_SLOW:
                        QPRINTF("[ADV]: Timed out from directed slow advertising.\r\n");
                        {
                            uint32_t err_code;
                    ...
                    case BLE_ADV_MODE_FAST:
                    {
                        uint32_t err_code;
                        m_adv_evt = BLE_ADV_EVT_FAST;
                        QPRINTF("[ADV]: Timed out from fast advertising, starting slow advertising.\r\n");
                    ...
                    case BLE_ADV_MODE_SLOW:
                        m_adv_evt = BLE_ADV_EVT_IDLE;
                        QPRINTF("[ADV]: Timed out from slow advertising, stopping advertising.\r\n");
                        if (m_evt_handler != NULL)
                        {
                            m_evt_handler(m_adv_evt);
                        }
