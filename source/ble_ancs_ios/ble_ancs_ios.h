...
#define ANCS_DATA_SIZE			ANCS_RECEIVE_DATA_SIZE
...
typedef struct
{
    ...
    uint8_t                         p_attr_data[ANCS_DATA_SIZE];  /**< Pointer to where the memory is allocated for storing incoming attributes. */
...
extern void ble_ancs_ccc_read(ble_ancs_c_t * p_ancs);
extern void Hang_up_Photo(bool action_state);
extern void ios_ancs_service_init(void);
